...
export function EstimatorFooter() {
  ...
  // Step 6 is the contact form, so "Next" should be handled by the form submit.
  const isFormStep = state.currentStep === 6;

  // Render nothing if it's the Thank You page
  ...
  return (
    <footer className="bg-background/95 supports-[backdrop-filter]:bg-background/60 border-border animate-in slide-in-from-bottom-6 fixed right-0 bottom-0 left-0 z-40 border-t p-4 shadow-2xl backdrop-blur duration-300 md:px-8 md:py-6">
      <div className="container mx-auto flex flex-col items-center justify-between gap-4 md:flex-row">
        ...
        <div className="order-1 flex w-full items-center gap-3 md:order-2 md:w-auto">
          {!isFirstStep && (
            <Button
              variant="outline"
              size="lg"
          ...
          {!isFormStep && (
            <Button
              ...
              onClick={handleNext}
            >
              {state.currentStep === 5 ? "Get Quote" : "Next Step"}
